...
3. [ ] Fill the map with all data from the game state (rejilla, characters, objects, etc.).
   ...
   3.4. [ ] Modify the `save_game_status` function in `server/game_data.py`. Since this function is called immediately upon receiving any new game data from the server, it's the perfect trigger. It will be updated to call `update_map_from_game_state` to ensure our `game_map` is dynamically updated in real-time.
