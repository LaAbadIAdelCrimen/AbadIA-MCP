...
6. [x] Implement auto-saving of the map when the player changes screens.
   6.1. [x] Modify the `update_map_from_game_state` function in `server/game_data.py`.
   6.2. [x] Before any updates are made, get the `numPantalla` from the incoming `game_status` and find Guillermo's `posX` and `posY`.
   6.3. [x] Check the `room` number currently stored in `game_map` at Guillermo's position.
   6.4. [x] If the new `numPantalla` is different from the stored `room` number (and the stored number is not 0), trigger a save by calling `save_map("current_map", game_map)`. Add logging to indicate that an auto-save has occurred due to a screen change.